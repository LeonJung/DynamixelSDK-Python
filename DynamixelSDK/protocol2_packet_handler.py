...
class Protocol2PacketHandler(object):
    ...
    def addStuffing(self, packet):
        ...
        for i in range(0, packet_length_in - 2): # except CRC
            ...
            index = index + 1
            if packet[i+PKT_INSTRUCTION] == 0xFD and packet[i+PKT_INSTRUCTION-1] == 0xFF and packet[i+PKT_INSTRUCTION-2] == 0xFF:
                # FF FF FD
                temp[index] = 0xFD
                index = index + 1
    ...
    def readRx(self, port, id, length, data=[], error=0):
        ...
        while True:
            result = self.rxPacket(port, rxpacket)
